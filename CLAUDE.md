...
- **收到实施方案/TDD 时不等于已评审**：即使用户给了完整的实施方案，仍然要先提取待确认设计决策，逐个提交用户确认后才能编码。方案越详细，隐含假设越多，越需要逐项确认（详见 DEV-17）
...
- **调研/搜索任务强制 checklist**（派 subagent 前必须走完，不允许跳步）：
  1. **拆子主题**：把调研问题拆成独立的子主题（如"质量保障"、"并行策略"、"调度机制"）
  2. **数关键词组**：列出每个子主题的搜索关键词
  3. **判断并行**：关键词组 ≥ 2 → 必须拆成多个并行 Task agent；只有 1 组 → 可以单 agent
  4. **并行发出**：在同一条消息中发出所有 Task 调用，不要串行等一个完了再发下一个
  5. **汇总输出**：所有 agent 返回后统一汇总，去重整合
  > 根因（DEV-16）：直觉是"派一个 agent 去查"，跳过了"先拆再派"。这个 checklist 把拆分变成强制前置步骤。
