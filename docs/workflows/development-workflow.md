...
**实现**：
1. 用 TeamCreate 创建五方团队，5 个角色 agent 常驻（团队在阶段 0~1 之间不释放）
2. 各角色基于用户提出的方向，发散讨论：备选方案、利弊、风险、范围边界
3. 汇总各方意见，提炼待用户拍板的关键决策
4. 用户拍板方向后，进入阶段 1

**产出**：讨论记录存入 `docs/specs/SPEC-XXX-功能名/讨论细节/R1-脑暴-YYYY-MM-DD.md`，关键决策汇总提交用户并同步到 `docs/discussions.md` 索引
...
2. IR 草稿写完后停下，发给阶段 0 的同一批五方 agent 评审（复用上下文，不重新拉起）
3. 五方评审聚焦：与第一轮结论是否一致、功能点遗漏、验收标准覆盖度、范围蔓延
4. 汇总评审意见，修订 IR
5. 用户确认 IR → 关闭五方团队（shutdown）→ 进入阶段 2
