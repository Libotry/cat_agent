...
> **记录规则**：本文件只记录跨前后端的通用教训（双终端分工、接口契约、工具调用策略、外部 CLI 集成、Git 流程、Code Review 流程等）。纯前端问题（CSS/组件/ARIA/React hooks）写 `error-book-dev-frontend.md`，纯后端问题（数据库/API/LLM/Agent）写 `error-book-dev-backend.md`。
