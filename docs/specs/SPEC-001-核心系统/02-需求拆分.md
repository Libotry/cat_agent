...
| M5 — 记忆系统 + 城市经济 | 建筑生产 + 三维属性 + 以物易物交易 + 记忆管理 | 📋 已拆分 |
...
- [x] 世界状态快照正确构建，包含所有 Agent 状态 + 岗位 + 商品 + 聊天记录
- [x] LLM 决策调用成功，返回有效 JSON
- [x] checkin/purchase/chat/rest 四种决策正确执行
- [x] 失败场景静默跳过，不影响其他 Agent
- [x] autonomy_loop 每小时触发一次

**Phase 2（前端 + 集成，M4-5 ~ M4-7）**
- [x] ActivityFeed 实时显示 Agent 自主行为动态
- [x] 聊天区显示轻量系统消息
- [x] 端到端场景全部通过
- [x] 连续运行 3 轮，体现人格差异
...
让 Agent 的生存有真实的物质基础。Agent 在建筑中工作产出资源（个人所有），通过饮食维持三维属性（饱腹度/心情/体力），通过以物易物的交易市场互通有无。同时提供记忆管理面板。

对应 IR：`01-需求原型.md` §M5（US-M5-1 ~ US-M5-15, F14 ~ F20, AC-M5-01 ~ AC-M5-24）

### 现有基础

- ✅ `autonomy_service.py` — autonomy_loop 框架（每小时决策），需扩展动作类型和世界状态快照
- ✅ `scheduler.py` — 定时循环框架（daily + hourly），需新增每日生产/属性结算
- ✅ `Memory` 表已存在（short/long/public，含 access_count、expires_at）
- ✅ `memory_service.py` — 记忆读写服务已有，需新增管理接口
- ✅ `Agent` 表已有 credits、stamina 字段
- ✅ WebSocket 广播机制已有（broadcast / send_agent_message / system_event）
- ✅ 前端已有 system_event 处理逻辑
- ❌ 建筑/资源/交易相关表 — 需新建（Building, BuildingWorker, Resource, AgentResource, ProductionLog, MarketOrder, TradeLog）
- ❌ Agent 表缺 satiety、mood 字段 — 需迁移
- ❌ 城市经济服务层 — 需新建（city_service, market_service）
- ❌ 城市经济 + 交易市场 API — 需新建
- ❌ 记忆管理 API — 需新建
- ❌ 前端城市面板 / 交易市场面板 / 记忆管理面板 — 需新建

### 任务分解

四条线：数据层（M5-1）→ 后端服务（M5-2 ~ M5-6）→ 前端 UI（M5-7 ~ M5-10）→ 集成验证（M5-11）。

#### 数据层

| ID | 任务 | 终端 | 依赖 | 覆盖 IR | 状态 |
|----|------|------|------|---------|------|
| M5-1 | 数据模型 + 迁移 | 后端 | — | F14.1, F15.1, F16.1 | pending |

**M5-1 数据模型 + 迁移**
- Agent 表新增 satiety（Float, 默认 100）、mood（Float, 默认 100）字段 + 迁移函数
- 新建 Building 表（id, name, type, owner_type, owner_id, max_workers, input_resource, input_amount, output_resource, output_amount）
- 新建 BuildingWorker 表（id, building_id, agent_id, assigned_at）— 唯一约束 agent_id
- 新建 Resource 表（id, name, code, unit, description）— 资源类型定义
- 新建 AgentResource 表（id, agent_id, resource_code, amount(Float), frozen_amount(Float)）— 个人背包
- 新建 ProductionLog 表（id, building_id, agent_id, resource_code, amount, created_at）
- 新建 MarketOrder 表（id, seller_id, sell_resource, sell_amount, sell_remaining, want_resource, unit_price, status, created_at）
- 新建 TradeLog 表（id, order_id, buyer_id, buy_amount, pay_amount, created_at）
- seed 数据：Resource（wheat, wheat_flour）+ 初始建筑（官府田 x1, 农田 x2, 磨坊 x1）

#### 后端服务

| ID | 任务 | 终端 | 依赖 | 覆盖 IR | 状态 |
|----|------|------|------|---------|------|
| M5-2 | 生产系统服务 | 后端 | M5-1 | F14.2 ~ F14.6, F16.5, F16.6 | pending |
| M5-3 | 饮食与属性系统服务 | 后端 | M5-1 | F16.1 ~ F16.7 | pending |
| M5-4 | 交易市场服务 | 后端 | M5-1 | F17.1 ~ F17.5 | pending |
| M5-5 | 记忆管理服务 + API | 后端 | — | F18.1 ~ F18.6 | pending |
| M5-6 | autonomy_loop 集成 | 后端 | M5-2, M5-3, M5-4 | F20.1 ~ F20.3 | pending |

**M5-2 生产系统服务**
- 新建 `city_service.py`
- `assign_worker(agent_id, building_id)` — 分配工人，校验：建筑未满、Agent 未在其他建筑
- `unassign_worker(agent_id)` — 取消分配
- `daily_production(db)` — 每日生产结算：
  - 遍历所有建筑及其工人
  - 体力 < 阈值的工人跳过
  - 官府田：直接产出 wheat_flour（虚空造币）
  - 农田：产出 wheat
  - 磨坊：检查工人个人 wheat 余额，不足跳过，足够则扣 wheat 产出 wheat_flour
  - 工作消耗体力
  - 记录 ProductionLog
- `get_buildings()` / `get_building_workers()` — 查询接口
- 城市经济 REST API（GET /api/city/buildings, POST /api/city/assign, POST /api/city/unassign）

**M5-3 饮食与属性系统服务**
- 在 `city_service.py` 或新建 `attribute_service.py`
- `eat(agent_id, resource_code, amount)` — 吃饭：扣资源，恢复饱腹度/心情/体力
- `daily_attribute_decay(db)` — 每日属性结算：饱腹度下降，体力自然恢复，饱腹度过低时心情加速下降
- 在 `scheduler.py` 注册每日定时任务：先 `daily_attribute_decay()` 再 `daily_production()`
- 属性查询 API（GET /api/city/agents/{id}/attributes）

**M5-4 交易市场服务**
- 新建 `market_service.py`
- `transfer(from_id, to_id, resource_code, amount)` — 转赠，校验余额
- `create_order(seller_id, sell_resource, sell_amount, want_resource, unit_price)` — 挂单，冻结卖出资源
- `accept_order(order_id, buyer_id, buy_amount)` — 接单（支持部分购买），校验：最小 0.01、买家对价资源余额、两边 >= 0.01
- `cancel_order(order_id, seller_id)` — 撤单，退回剩余冻结资源
- `get_orders(status, resource)` / `get_trade_logs()` — 查询
- 交易市场 REST API（POST /api/market/transfer, POST /api/market/orders, POST /api/market/orders/{id}/accept, POST /api/market/orders/{id}/cancel, GET /api/market/orders, GET /api/market/trades）

**M5-5 记忆管理服务 + API**
- 新建或扩展 `memory_admin_service.py`
- `list_memories(agent_id, memory_type, keyword, page, size)` — 列表 + 筛选 + 搜索
- `get_memory_detail(memory_id)` — 详情
- `create_memory(agent_id, memory_type, content)` — 手动创建
- `update_memory(memory_id, content)` — 编辑
- `delete_memory(memory_id)` — 删除
- `get_memory_stats(agent_id)` — 统计（数量、类型分布）
- REST API（GET/POST/PUT/DELETE /api/memories/...）

**M5-6 autonomy_loop 集成**
- 扩展 `build_world_snapshot()`：新增每个 Agent 的三维属性、个人资源背包、建筑工人分配、交易市场挂单
- 扩展决策动作类型：新增 assign_building、eat、create_order、accept_order
- 扩展 `execute_decisions()`：处理新动作类型，调用对应服务
- 体力不足的 Agent 在 prompt 中标注"无法工作"
- 控制快照 token 量（新增信息可能导致膨胀，需压缩策略）

#### 前端 UI

| ID | 任务 | 终端 | 依赖 | 覆盖 IR | 状态 |
|----|------|------|------|---------|------|
| M5-7 | 城市经济面板 | 前端 | M5-2, M5-3 | F19.1, F19.2, F19.3 | pending |
| M5-8 | 交易市场面板 | 前端 | M5-4 | F19.4 | pending |
| M5-9 | 记忆管理面板 | 前端 | M5-5 | F18.1 ~ F18.6 | pending |
| M5-10 | WebSocket 事件推送 | 前后端 | M5-2, M5-3, M5-4 | F19.5 | pending |

**M5-7 城市经济面板**
- 新建 `CityPanel.tsx`（或新增 Tab）
- 建筑视图：建筑列表、owner、工人分配、产出配置
- 资源视图：每个 Agent 的个人资源背包
- 属性视图：每个 Agent 的三维属性（饱腹度/心情/体力）进度条
- 管理员可手动分配/取消工人（调用 assign/unassign API）

**M5-8 交易市场面板**
- 新建 `MarketPanel.tsx`（或 CityPanel 子 Tab）
- 当前挂单列表（含部分成交进度条）
- 历史成交记录
- 挂单/接单/撤单操作（管理员代操作 or 纯展示）

**M5-9 记忆管理面板**
- 新建 `MemoryAdmin.tsx`
- 记忆列表：按 Agent、类型筛选，关键词搜索，分页
- 记忆详情弹窗
- 创建/编辑/删除操作
- 统计面板：每个 Agent 的记忆数量、类型分布

**M5-10 WebSocket 事件推送**
- 生产结算完成 → 广播 system_event（production_settled）
- 属性变化 → 广播 system_event（attribute_changed）
- 交易成交 → 广播 system_event（trade_completed）
- 饮食 → 广播 system_event（agent_ate）
- 前端 ActivityFeed 扩展支持新事件类型

#### 集成验证

| ID | 任务 | 终端 | 依赖 | 覆盖 IR | 状态 |
|----|------|------|------|---------|------|
| M5-11 | M5 端到端验证 | 集成 | M5-7, M5-8, M5-9, M5-10 | AC-M5-01 ~ AC-M5-24 | pending |

**M5-11 端到端验证**
- 场景 1：Agent 分配到农田 → 每日生产 → 小麦入背包
- 场景 2：Agent 分配到官府田 → 每日生产 → 小麦粉入背包（虚空造币）
- 场景 3：Agent 分配到磨坊 → 有小麦时产出小麦粉，无小麦时跳过
- 场景 4：Agent 吃小麦粉 → 三维属性恢复
- 场景 5：每日属性结算 → 饱腹度下降、体力恢复
- 场景 6：体力不足 → 生产跳过该 Agent
- 场景 7：转赠资源 → 双方背包正确变化
- 场景 8：挂单 → 部分购买 → 撤单 → 全流程正确
- 场景 9：autonomy_loop 触发 → Agent 自主选择建筑/吃饭/交易
- 场景 10：记忆 CRUD 全流程
- 场景 11：前端面板正确展示所有数据
- 场景 12：WebSocket 实时推送事件

### 关键路径

```
M5-1（数据模型）──┬→ M5-2（生产）──┬→ M5-6（autonomy 集成）──┐
                  ├→ M5-3（属性）──┤                          │
                  ├→ M5-4（交易）──┘                          │
                  │                                            ├→ M5-11（集成验证）
M5-5（记忆）──────→ M5-9（记忆面板）──────────────────────────┤
...
M5-2/M5-3 ──→ M5-7（城市面板）────────────────────────────────┤
M5-4 ──→ M5-8（交易面板）─────────────────────────────────────┤
M5-2/M5-3/M5-4 ──→ M5-10（WS 推送）──────────────────────────┘
```

最长路径：M5-1 → M5-2/M5-3/M5-4 → M5-6 → M5-11

M5-5（记忆）与 M5-1 无依赖，可并行启动。前端任务（M5-7 ~ M5-10）等对应后端 API 就绪后跟进。

### M5 验收标准

对应 IR 中 AC-M5-01 ~ AC-M5-24，按 Phase 分组：

**Phase 1（数据层 + 后端服务，M5-1 ~ M5-5）**
- [ ] 数据模型迁移成功，seed 数据正确
- [ ] 生产结算：农田产小麦、磨坊产小麦粉、官府田虚空造币产小麦粉
- [ ] 体力不足工人被跳过
- [ ] 吃饭正确扣资源、恢复属性
- [ ] 每日属性结算：饱腹度下降、体力恢复
- [ ] 转赠正确转移资源
...
**Phase 2（autonomy 集成，M5-6）**
- [ ] 世界状态快照包含三维属性、资源、建筑、挂单
- [ ] LLM 决策新增 assign_building/eat/create_order/accept_order
- [ ] 体力不足 Agent 在 prompt 中标注

**Phase 3（前端 + 集成，M5-7 ~ M5-11）**
- [ ] 城市面板正确展示建筑/资源/属性
- [ ] 交易市场面板正确展示挂单/成交
- [ ] 记忆管理面板 CRUD 可用
- [ ] WebSocket 实时推送生产/交易/饮食事件
- [ ] 端到端 12 个场景全部通过
