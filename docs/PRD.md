...
| Embedding 模型 | BAAI/bge-small-zh-v1.5 | 已实施 | 中英双语，512 维，体积可控 |

### 技术栈详情
- **后端**: FastAPI + Uvicorn（ASGI）+ WebSocket 实时通信 + APScheduler
- **前端**: React + Vite（第一阶段纯 UI，未来按需引入 PixiJS 做 2D 地图）
- **数据库**: SQLite（结构化）+ LanceDB（向量）
- **AI 集成**: OpenAI/Anthropic SDK + sentence-transformers（bge-small-zh-v1.5）
...
### M1: 基础聊天系统 ✅ (2026-02-14)
- Agent CRUD API
- WebSocket 实时聊天
- 唤醒引擎（@必唤 + 小模型选人 + 定时触发）
- LLM 集成（OpenAI/Anthropic/OpenRouter）
- 链式唤醒（最大深度 3 层）

### M1.5: Bot 接入 ✅ (2026-02-14)
- OpenClaw Plugin（TypeScript channel plugin）
- 约 300 行代码，6 个文件
...
**Phase 1 ✅ (2026-02-15)**
- LanceDB 向量存储（bge-small-zh-v1.5）
- 经济服务（信用点、额度检查、转账）
- LLM 用量追踪（tokens/cost）
- 发言频率控制
